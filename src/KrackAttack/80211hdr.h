...
//
//
//ryan - probe request and rate

typedef struct DOT11_RATE {
	USHORT				Rate : 7;
	USHORT				Type : 1;
} DOT11_RATE, *PDOT11_RATE;

typedef struct DOT11_BASIC_RATE {
	DOT11_INFO_ELEMENT	ieheader;
	UCHAR				rate[8]; //the number of rates is up to the user
} DOT11_BASIC_RATE, *PDOT11_BASIC_RATE;

typedef struct DOT11_EXT_RATE_RATE {
	DOT11_INFO_ELEMENT	ieheader;
	UCHAR				rate[4];
} DOT11_EXT_RATE, *PDOT11_EXT_RATE;

typedef struct DOT11_POWER_FRAME {
	DOT11_INFO_ELEMENT	ieheader;
	USHORT				MinTransmit : 8;
	USHORT				MaxTransmit : 8;
} DOT11_POWER_FRAME, *PDOT11_POWER_FRAME;

typedef struct DOT11_CHANNEL_FRAME {
	DOT11_INFO_ELEMENT	ieheader;
	USHORT				FirstChannel : 8;
	USHORT				NumChannel : 8;
} DOT11_CHANNEL_FRAME, *PDOT11_CHANNEL_FRAME;


typedef struct DOT11_PROBE_REQUEST {
	//Information Elements
	DOT11_INFO_ELEMENT  ssid;
	DOT11_BASIC_RATE 	rateframe;
} DOT11_PROBE_REQUEST, *PDOT11_PROBE_REQUEST;
...
typedef struct DOT11_ASSOC_REQUEST_FRAME {
	USHORT Capability;//DOT11_CAPABILITY Capability;
